...
So if we have any ETL tool and we need to add the functionality of preview data then we can use apache livy for faster execution with the help of spark context sharing. In the spark sharing context we can save the time to only one time to create the yarn application no need for every preview 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU2MDc2NzA1NiwxNDE3NzI1MjMxLC01Mz
