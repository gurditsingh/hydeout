...
**Lets understand state with Example**![window events](https://github.com/gurditsingh/blog/blob/gh-pages/_screenshots/join_stream.jpg?raw=true)
 ...
 - Batch 2 have only one start event with id **R2** but batch 2 don't have end event so join does not generates any output. Spark store start event in start state and wait for end event in 

 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg4OTU4NTAyNywtMTk0NDY3NzQ0MCwxNj
